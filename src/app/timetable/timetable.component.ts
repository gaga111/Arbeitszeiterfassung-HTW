 ...
 /*,sa
  ...
  parseUserTimes(time, mon) {
    ...
    for (var i = 0; i < time.length; i++) {
      if (time[i]['Jahr'] == this.currentYear && time[i]['Monat'] == mon) { //Aktueller Monat
        ...
        var start, ende, untrbr, istval;
        if (datum == this.currentDate.toString()) {
          console.log("HEUTE");

          start = (<HTMLInputElement>document.getElementById("beginnHeute"));
          ende = (<HTMLInputElement>document.getElementById("endeHeute"));
          untrbr = (<HTMLInputElement>document.getElementById("untrbrHeute"));
          istval = (<HTMLInputElement>document.getElementById("spalteIstHeute"));
        } else {
          start = (<HTMLInputElement>document.getElementById(`${"beginn" + time[i]['Tag'] + "." + time[i]['Monat']}`));
          ende = (<HTMLInputElement>document.getElementById(`${"ende" + time[i]['Tag'] + "." + time[i]['Monat']}`));
          untrbr = (<HTMLInputElement>document.getElementById(`${"untrbr" + time[i]['Tag'] + "." + time[i]['Monat']}`));
          istval = (<HTMLInputElement>document.getElementById(`${"spalteIst" + time[i]['Tag'] + "." + time[i]['Monat']}`));
        }
        if (time[i]['Soll']) {
  ...
  setStartTime(time: String, datum: String) {
    ...
    if (datum == "Heute") {
      ...
      ende = <HTMLInputElement>document.getElementById("ende" + `${datum}`);
    }
  ...
  berechneDiff(ende, datum) {
    ...
    //console.log(diff);
    /*if( datum.split(':')[1] == this.todayMonth -1 ){
    if (datum == this.currentDate.toString()){
      datum = "Heute";
     }
    }*/
  ...
  speichern(datum) {
    ...
    ende.disabled = true;
    var untrbr = <HTMLInputElement>document.getElementById(`${"untrbr" + id}`);
    untrbr.disabled = true;
    if (id == "Heute") {
  ...
  checkRowRules(id: String, indx) {
    ...
    console.log(document.getElementById(`${"ausf" + id}`));
